...
// trace_macros!(true);
use std::{collections::BTreeMap, time::Instant, sync::Arc};
...
fn bench_get_backends(
    ...
    c.bench_function("bench get TrieCollection<Mem> (no-cache)", |b| {
        use triedb::empty_trie_hash;
        use triedb::gc::MapWithCounter;

        let handle = MapWithCounter::default();

        let collection = TrieCollection::new(Arc::new(handle));
    ...
    c.bench_function("bench get TrieCollection<Rocks> (no-cache)", |b| {
        use triedb::empty_trie_hash;
        use triedb::rocksdb::{merge_counter, DB};
