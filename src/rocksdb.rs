...
impl<'a, D: Borrow<DB>> DbCounter for RocksDatabaseHandle<'a, D> {
    // Insert value into db.
    // Check if value exist before, if not exist, increment child counter.
    fn gc_insert_node<F>(&self, key: H256, value: &[u8], mut child_extractor: F)
    where
        F: FnMut(&[u8]) -> Vec<H256>,
    {
        ...
        retry! {
                ...
                .is_none()
            {
                trace!("inserting node {}=>{:?}", key, node);
                for hash in &childs {
                    self.increase(&mut tx, *hash)?;
                }

                tx.put(key.as_ref(), value)?;
                self.create_counter(&mut tx, key)?;
                tx.commit()?;
            }
        }
    }
    fn gc_count(&self, key: H256) -> usize {
        let db = self.db.borrow();
        let mut tx = db.transaction();
        self.get_counter_in_tx(&mut tx, key)
            .expect("Cannot read value") as usize
    }
        ...
        F: FnMut(&[u8]) -> Vec<H256>,
    {
        ...
        retry! {
            ...
            let mut tx = db.transaction();
            if let Some(value) = tx.get_for_update(key.as_ref(), EXCLUSIVE)? {
                let count = self.get_counter_in_tx(&mut tx, key)?;
                if count > 0 {
                    trace!("ignore removing node {}, counter: {}", key, count);
                    return Ok(vec![]);
                }
                tx.delete(&key.as_ref())?;
                self.remove_counter(&mut tx, key)?;
                ...
                for hash in &childs {
                    let child_count = self.decrease(&mut tx, *hash)?;
                    if child_count <= 0 {
                        nodes.push(*hash);
                    }
                }
